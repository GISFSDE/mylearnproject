package pers.lxl.mylearnproject.programbase.algorithm.datastructure.linkedlist;
...
public class singleLinkedList<T> {
        ...
        private class Node {
            ...
            public void remove(Node previous, int index) {
                ...
                if (singleLinkedList.this.foot++ == index) {
                    previous.next = this.next;    //this表示当前要删除的节点
                    this.next = null;
                    singleLinkedList.this.count--;
                    return;
                } else {
            ...
            public void replace(int index, T newData) {
                if (singleLinkedList.this.foot++ == index) {    //找到了某个值的索引和传入的索引相同，直接替换
                    this.data = newData;
                } else {
